function plot_mean_and_scatter_3v2(datasets_to_plot,x_values,color_vals,figure_toggle, std_toggle,sem_toggle,ci_toggle, batch_num)
...
for ii =1:length(datasets_to_plot)
    ...
    hold on
    scatter(x_values(ii).*ones(1,length(metric_to_plot(batch_num==2))),metric_to_plot(batch_num==2),[],color_vals(ii,:),'v','filled', 'jitter', 'on', 'jitterAmount', 0.3);
    hold on
    scatter(x_values(ii).*ones(1,length(metric_to_plot(batch_num==3))),metric_to_plot(batch_num==3),[],color_vals(ii,:),'d','filled', 'jitter', 'on', 'jitterAmount', 0.3);
    hold on
